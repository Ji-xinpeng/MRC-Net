...
def get_dectect_dataloader():
    trans_train, trans_test = define_spatial_transform()
    train_dectect_dataset = Dataset_dectect_video(get_pkl_path(), "dectect_train", trans_train)
    train_dectect_dataloader = DataLoader(train_dectect_dataset, batch_size=params['batch_size'], shuffle=True,
                                    num_workers=params['num_workers'])

    val_dectect_dataset = Dataset_dectect_video(get_pkl_path(), "dectect_val", trans_test)
    val_dectect_dataloader = DataLoader(val_dectect_dataset, batch_size=params['batch_size'], num_workers=params['num_workers'])

    return train_dectect_dataloader, val_dectect_dataloader



def get_dectect_model_policies(model):
    first_conv_weight = []
    first_conv_bias = []
    normal_weight = []
    normal_bias = []
