...
class Action(nn.Module):
    def __init__(self, net, n_segment=3, shift_div=8):
        ...
        self.action_p3_conv1 = nn.Conv2d(self.in_channels, self.in_channels, kernel_size=(3, 3),
        ...
        self.channel_aaten = attention.Attention(self.n_segment, num_heads=args.num_heads, qkv_bias=False,
                                                 qk_scale=None, attn_drop_ratio=0., proj_drop_ratio=0.)
        self.local_attention = LocalAttention(self.in_channels, self.n_segment)
        self.spatial_attention = attention.Attention(self.reduced_channels, num_heads=args.num_heads, qkv_bias=False,
                                                     qk_scale=None, attn_drop_ratio=0., proj_drop_ratio=0.)
    ...
    def forward(self, x):
        ...
        _, x3_plus1 = x3.view(n_batch, self.n_segment, c, h, w).split([1, self.n_segment - 1], dim=1)
        x_p3 = x3_plus1 - x3_plus0
        x_p3 = F.pad(x_p3, self.pad, mode="constant", value=0)
        x_p3 = x_p3.reshape(nt, c, h, w)
        x_local_attention = self.local_attention(x_p3)  # nt, c ,1, 1
        x_p3 = x_p3 * x_local_attention
        x_p3 = self.action_p3_conv1(x_p3)
        x_p3 = x_p3 + x_shift

        # 这里可以有两种方式，全局的自注意力是用运动相减后的，还是用原始的
